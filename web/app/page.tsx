...
type BattleState = {
  ...
  lightBarrierRemaining: number; members: PartyBattleState[];
...
export default function Home() {
  ...
  return (
    <main className="game-shell">
      <BattleScene barrierRatio={barrierRatio} bossHealthRatio={bossHealthRatio} groggyRemaining={battle.groggyRemaining} lightBarrierRemaining={battle.lightBarrierRemaining} />
      ...
      <section className="party-hud" aria-label="원정대 상태">
        ...
        <div className="party-list">{PARTY.map((member) => {
          ...
          return <div className="party-member" key={member.id} aria-label={`${member.name} HP ${Math.ceil(state.hp)}, 보호막 ${Math.ceil(state.shield)}`}><span className={`party-avatar role-${member.id}`} style={{ '--member-color': member.color } as CSSProperties}><i /><b>{member.name.slice(0, 1)}</b></span><div className="party-health"><div><b>{member.name}</b><span>{member.roleLabel}</span></div><div className="member-bars"><div className="member-health-track"><i style={{ width: `${state.hp}%` }} /></div>{state.shield > 0 && <div className="member-shield-track" aria-label={`보호막 ${Math.ceil(state.shield)}`}><i style={{ width: `${shieldRatio}%` }} /></div>}</div></div></div>;
        })}</div>
      </section>
